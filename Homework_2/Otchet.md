...
В ходе работы была реализована потокобезопасная очередь задач с использованием `std::mutex` и `std::condition_variable`. Четыре рабочих потока параллельно обрабатывают 20 задач примерно за 5 секунд вместо 20 
